...
char *expand_double_quote(t_tools *tools, const char *current_word, size_t *j, int heredoc_flag)
{
    ...
    while (current_word[*j] && current_word[*j] != '"')
    {
        if (current_word[*j] == '$' && heredoc_flag == 0)
        {
            ...
            {
                char *exit_status = ft_itoa(tools->exit_status);
            ...
            else if (current_word[*j] && current_word[*j] != ' ' && current_word[*j] != '"')
            {
                var_start = *j;
            ...
            {
                char *new_expanded_word = ft_strjoin(expanded_word, "$");
                free(expanded_word);
                expanded_word = new_expanded_word;
            }
        }
...
char *expand_vars(t_tools *tools, char *current_word, int *flag, int heredoc_flag)
{
    ...
    while (j < len)
    {
        ...
        else if (current_word[j] == '$' && heredoc_flag == 0)
        {
            ...
            else if (j + 1 < len && (current_word[j + 1] == '?' || ft_isalnum(current_word[j + 1]) || current_word[j + 1] == '_'))
            {
